		...
		<form action="update_cart" method="post" style="max-width: 600px; margin: 0 auto">
			<c:forEach items="${cart.items}" var="item" varStatus="status">
				<div class="row border rounded p-1">
					...
					<div class="col-5">
						<img class="img-fluid" src="${item.key.card.imageUrl}" />
					</div>
					
					<div class="col-6">
						<div><h5>${item.key.card.cardName}</h5></div>
						
						<div>
							<input type="hidden" name="listingId" value="${item.key.listingId}" />
							<input type="number" name="quantity${status.index + 1}" value="${item.value}" 
								class="form-control" style="max-width: 50px" size="5" required step="1" min="1" max=${item.key.quantity } />
						</div>
						...
						<div>
							<a href="remove_from_cart?listingId=${item.key.listingId}">Remove</a>
						</div>
					</div>
				</div>
				<div class="row">&nbsp;</div>
